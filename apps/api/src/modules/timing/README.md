...
Phase 4B adds the separate `time-behavior-v1` aggregation policy in `time-behavior-policy.ts`. It defines shared pressure/ample/fast-move thresholds, rating bands, coverage/evidence gates, and deterministic helpers consumed by later diagnosis aggregates. It does not reinterpret source clocks or implement diagnosis findings. See `docs/time-behavior-analysis.md`.

Issue #59 is the first concrete consumer of that policy. `TIME-001` reads current derived before-move clocks through the diagnosis repository, rejects incomplete/unreliable game timing from recurrence denominators, and keeps exact-control/increment identity visible without redefining source timing semantics. See `docs/time-pressure-exposure.md`.
