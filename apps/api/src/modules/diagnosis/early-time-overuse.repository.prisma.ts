...
import {
  ...
  type EarlyTimeOveruseAnalysisRun,
  type EarlyTimeOverusePhase,
  type EarlyTimeOveruseRepository,
  type EarlyTimeOveruseScope,
  type EarlyTimeOveruseSourceGame,
} from './early-time-overuse.service';
...
function phaseValue(value: string | null): EarlyTimeOverusePhase | null {
...
): EarlyTimeOveruseAnalysisRun | null {
  if (
    !run
...
export const prismaEarlyTimeOveruseRepository: EarlyTimeOveruseRepository = {
  ...
  async loadCandidates(appUserId, scope) {
    const rows = await prisma.importedGame.findMany({
      ...
      take: EARLY_TIME_OVERUSE_MAX_CANDIDATE_GAMES + 1,
      select: {
        id: true,
        variant: true,
        speedCategory: true,
    ...
    return rows.map<EarlyTimeOveruseSourceGame>((row) => {
