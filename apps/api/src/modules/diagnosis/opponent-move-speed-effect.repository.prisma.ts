...
import {
  ...
  type OpponentMoveSpeedEffectAnalysisRun,
  type OpponentMoveSpeedEffectPhase,
  type OpponentMoveSpeedEffectRepository,
  type OpponentMoveSpeedEffectScope,
  type OpponentMoveSpeedEffectSourceGame,
} from './opponent-move-speed-effect.service';
...
function phaseValue(value: string | null): OpponentMoveSpeedEffectPhase | null {
...
function phaseAtBoundary(
  ...
  boundaryPly: number,
): OpponentMoveSpeedEffectPhase | null {
  return ranges.find(
...
): OpponentMoveSpeedEffectAnalysisRun | null {
  if (
    !run
...
export const prismaOpponentMoveSpeedEffectRepository: OpponentMoveSpeedEffectRepository = {
  ...
  async loadCandidates(appUserId, scope) {
    const rows = await prisma.importedGame.findMany({
      ...
      take: OPPONENT_MOVE_SPEED_EFFECT_MAX_CANDIDATE_GAMES + 1,
      select: {
        id: true,
        variant: true,
        speedCategory: true,
    ...
    return rows.map<OpponentMoveSpeedEffectSourceGame>((row) => {
      ...
      return {
        ...
        plies: row.plies.map((ply) => ({
          plyNumber: ply.plyNumber,
          isUserMove: ply.isUserMove,
          moveTimeCentiseconds: ply.clockDeltaMoveTimeCentiseconds,
