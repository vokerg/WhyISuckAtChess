...
import {
  ...
  type PlayedTooFastAnalysisRun,
  type PlayedTooFastPhase,
  type PlayedTooFastRepository,
  type PlayedTooFastScope,
  type PlayedTooFastSourceGame,
} from './played-too-fast.service';
...
function phaseValue(value: string | null): PlayedTooFastPhase | null {
...
export const prismaPlayedTooFastRepository: PlayedTooFastRepository = {
  ...
  async loadCandidates(appUserId, scope) {
    const rows = await prisma.importedGame.findMany({
      ...
      take: PLAYED_TOO_FAST_MAX_CANDIDATE_GAMES + 1,
      select: {
        id: true,
        variant: true,
        speedCategory: true,
    ...
    return rows.map<PlayedTooFastSourceGame>((row) => {
